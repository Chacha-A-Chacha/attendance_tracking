...
from utils.session_mapper import normalize_session_time, get_session_by_time, find_available_session
    ...
    try:
        for _, row in df.iterrows():
            ...
            if not saturday_session or not sunday_session:
                errors.append(f"Invalid session for {name}: Saturday={saturday_time}, Sunday={sunday_time}")
                continue
            ...
            if saturday_session.id != final_saturday_session.id:
                reassignments.append(f"{name}: Saturday session changed from {saturday_session.time_slot} to {final_saturday_session.time_slot}")
                
            if sunday_session.id != final_sunday_session.id:
                reassignments.append(f"{name}: Sunday session changed from {sunday_session.time_slot} to {final_sunday_session.time_slot}")
